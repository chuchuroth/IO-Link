...
int iolink_cycle(uint8_t pd_out, uint8_t *pd_in)
{
    ...
    /* ------------------------------------------------------------------ */
    /* 3a. Flush UART echo bytes.                                          */
    /*     TIOL221EVM echoes the 3 TX bytes back on DOUT/RX while EN=HIGH. */
    /*     Those bytes sit in the UART RX FIFO and must be drained before  */
    /*     we can receive the real 2-byte device D-sequence response.      */
    /* ------------------------------------------------------------------ */
    uint8_t echo[3];
    HAL_UART_Receive(&huart4, echo, 3, 3);   /* discard — timeout OK */
